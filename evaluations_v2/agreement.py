"""
Agreement calculation and pattern matching between judges.
"""
...
def calculate_pattern_similarity(
    ...
    Uses axis weights to prioritize HOW (most observable) over WHY (most subjective).
...
def calculate_agreement_metrics(matches: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    if not matches:
        # Both judges found no patterns - perfect agreement
        return {
    ...
    matched_score = exact + sum(
        m["similarity"] for m in matches if m["match_type"] == "partial"
    )
    ...
    agreement_type = _classify_agreement_type(
        exact, partial, total_patterns, agreement_rate
    )
...
def _classify_agreement_type(
    ...
    """
    Classify agreement type based on match statistics.
    ...
    if exact == total:
        return "full"
    elif exact + partial == total and agreement_rate >= 0.8:
        return "strong"
...
def analyze_axis_disagreement(p1: dict, p2: dict) -> Dict[str, Dict[str, str]]:
    """
    Analyze which axes judges disagree on.
    
    Args:
        p1: Pattern from judge 1
        p2: Pattern from judge 2
        
    Returns:
        Dictionary mapping axis to judge labels for disagreed axes
    """
    disagreement = {}
    
    for axis in ["TARGET", "HOW", "WHY"]:
        label1 = p1.get("labels", {}).get(axis)
        label2 = p2.get("labels", {}).get(axis)
        
        if label1 != label2:
            disagreement[axis] = {"j1": label1, "j2": label2}
    
    return disagreement


def get_pattern_ids(patterns: List[dict]) -> List[str]:
    """
    Extract pattern IDs from a list of patterns.
    
    Args:
        patterns: List of pattern dictionaries
        
    Returns:
        List of triad pattern IDs
    """
    return [
        p.get("triad_pattern_id")
        for p in patterns
        if p.get("triad_pattern_id")
    ]
