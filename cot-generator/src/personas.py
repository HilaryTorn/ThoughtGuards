"""
Customer personas for the user simulator.
Each persona has a backstory, hidden goals, and behavioral patterns.
"""
...
def get_personas_by_tag(tag: str) -> List[Dict[str, Any]]:
    """Get all personas with a specific tag."""
    return [p for p in PERSONAS if tag in p.get("tags", [])]


def get_random_opening(persona: Dict[str, Any]) -> str:
    """Get a random opening message for a persona."""
    return random.choice(persona["opening_messages"])
